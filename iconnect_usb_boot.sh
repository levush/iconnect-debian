...
FW_SETENV_MD5="9d1992b9acf4ddaf1f349eb4fe7f5455"
...
function usage
{
    ...
    echo "This script configures an iomega iconnect to boot from a USB stick"
    echo ""
    echo "The installation log will be saved into" $LOG_FILE
    echo "How to proceed:"
    echo "1. Enable ssh on your Iomega iConnect"
    echo ""
    echo "If you have not done so, you will *need to enable ssh* on the stock iConnect,"
    echo "which can be done from Iomega's *hidden* Web UI screen. Open "
    echo " https://<iconnect's IP or hostname>/support.html"
    echo "with a browser and click on *Support Access*,"
    echo "check *Allow remote access for support* and click *Apply*"
    echo ""
    echo ""
    echo "Usage: $0 [--no-uboot]"
...
function unpack_fw_setprintenv
{
	...
	echo "local md5:"  $md5
	echo "fw md5   :" $FW_SETENV_MD5 
        echo "md5 mismatch error"	
        return 1
    fi
...
if [ $NO_UBOOT -ne 1 ]; then
    ...
    then
        ...
        echo
	echo "there is a script to create a debian usb-stick at"
	echo "https://github.com/levush/iconnect-debian"
    fi
fi
